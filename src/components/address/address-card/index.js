...
import { alpha, Badge, styled, Typography } from "@mui/material";
import {
  CustomPaperBigCard,
...
export const AddressTypography = styled(Typography)(({ fontWeight }) => ({
...
const AddressCard = (props) => {
  ...
  useSelector((state) => state.addressModel);
  const handleClick = () => {
    setEditAddress(item);
    ...
    >
      <CustomStackFullWidth spacing={2}>
        ...
        >
          ...
          {item?.is_primary === 1 && (
            <Badge badgeContent={"Primary"} color="success" />
          )}
