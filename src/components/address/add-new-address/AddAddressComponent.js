...
const AddAddressComponent = ({
  ...
  const [location, setLocation] = useState({lat: editAddress?.latitude, lng: editAddress?.longitude})
  ...
  const updateAddress = () => {
    return editAddress?.is_primary === 1;
  };
  const [primaryKey, setPrimary] = useState(
    editAddress ? updateAddress() : false
  );
  ...
  useEffect(() => {
    if (placeDetails) {
      ...
      setLocation({ lat: editAddress?.latitude, lng: editAddress?.longitude });
    }
  }, [placeDetails,editAddress])
  useEffect(() => {
    const editLatLng = { lat: editAddress?.latitude, lng: editAddress?.longitude };
    dispatch({
