...
const firstBackground = (isRtl, theme) => {
  `linear-gradient(270deg, ${
    isRtl === "rtl" ? "rgba(255, 255, 255, 0)" : theme.palette.neutral[100]
  } 0%, ${
    isRtl === "rtl"
...
const secondBackground = (isRtl, theme) => {
  `linear-gradient(${isRtl === "rtl" ? "to left" : "to right"},  ${
    isRtl === "rtl"
      ? "rgba(255, 255, 255, 0)"
      : "rgba(75, 86, 107, 0.05) -28.57%, rgba(255, 255, 255, 0) 122.62%"
  } 0%, ${
    isRtl === "rtl" ? theme.palette.neutral[100] : "rgba(255, 255, 255, 0)"
  }  100%)`;
};
const rightBackground = (right, isRtl, theme) => {
  right === "true"
    ? firstBackground(isRtl, theme)
    : secondBackground(isRtl, theme);
};
const ButtonContainer = styled(Box)(
  ({ theme, right, isdisabled, noBackground, isRtl, rightSpace }) => ({
    ...
    background:
      noBackground === "true" ? null : rightBackground(right, isRtl, theme),
    zIndex: 1,
...
export const NextFood = ({
  ...
  const displayInMobile = displayNoneOnMobile || false;
  const theme = useTheme();
  const isSmall = useMediaQuery(theme.breakpoints.down("sm"));
  const displayNone = isSmall ? displayInMobile : false;
  return (
    <ButtonContainer
