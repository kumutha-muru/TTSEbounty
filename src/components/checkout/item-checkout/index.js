...
import useGetVehicleDetails from "../../../api-manage/hooks/react-query/order-place/useGetVehicleDetails";
...
const ItemCheckout = (props) => {
 ...
 /*  const [offlinePayments, setOfflinePayments] = useState(""); */
  ...
  const { data:addressData, refetch:addressFetch } = useGetAddressList()
  const paddingStyle = isSmall ? "0px" : "1.25rem";
  useEffect(() => {
  ...
  const userprofile = {
    contact_person_name: profileInfo?.f_name || "",
    contact_person_email: profileInfo?.email || "",
    contact_person_number: profileInfo?.phone || "",
  };
  useEffect(() => {
    const primaryAddress = addressData?.addresses.filter(function (value) {
    ...
    if (token && primaryAddress) {
      ...
      /*  const city = localStorage.getItem("city");
       ...
       setAddress({
         ...userprofile,
         latitude: guestUserInfo?.latitude.toString() || currentLatLng?.lat.toString(),
         longitude: guestUserInfo?.longitude.toString() || currentLatLng?.lng.toString(),
         address: guestUserInfo?.address || location,
         address_type: guestUserInfo?.address_type || "Selected Address",
         city: guestUserInfo?.city || fullAddress[fullAddress.length - 4],
         district:
           guestUserInfo?.district || fullAddress[fullAddress.length - 3],
         state: guestUserInfo?.state || fullAddress[fullAddress.length - 2],
         country: guestUserInfo?.country || fullAddress[fullAddress.length - 1],
         pincode: guestUserInfo?.pincode || pincode,
       });
     }
     refetch();
  }, [addressData]);
  ...
  } = useGetVehicleCharge({ tempDistance,storeId });
  const {
    data: vehicleData,
    isLoading: vehicleDataLoading,
    refetch: vehicleDataRefetch,
  } = useGetVehicleDetails({ tempDistance,storeId });
  useEffect(() => {
    if (distanceData) {
      extraChargeRefetch();
    }
  ...
  const foodProductVariations = (cart) => {
    return cart?.food_variations?.length > 0
  ...
  const withOutFoodProductVariations = (cart) => {
    return cart?.selectedOption?.length > 0 ? cart?.selectedOption : [];
  };
      ...
      return {
        ...
        variation:
          cart?.module_type === "food"
            ? foodProductVariations(cart)
            : withOutFoodProductVariations(cart),
      };
    });
  };
  const moduleWiseFormData = (
    carts,
    productList,
    guestId,
    isDigital,
    ...
    return {
      ...
      house: address?.house,
      floor: address?.floor,
      city: token ? address?.city : guestUserInfo?.city,
      district: token ? address?.district : guestUserInfo?.district,
      state: token ? address?.state : guestUserInfo?.state,
      country: token ? address?.country : guestUserInfo?.country,
      pincode: token ? address?.pincode : guestUserInfo?.pincode,
    };
  };
  const contactPersonEmail = token
    ? address?.contact_person_email
    : guestUserInfo?.contact_person_email;
  const contactPersonNumber = token
    ? address?.contact_person_number
    : guestUserInfo?.contact_person_number;
  const returnFormData = (
    ...
    formData.append(
      "address",
      token ? address?.address : guestUserInfo?.address
    );
    formData.append(
      "address_type",
      token ? address?.address_type : guestUserInfo?.address_type
    );
    formData.append("lat", token ? address?.lat : guestUserInfo?.latitude);
    formData.append(
      "latitude",
      token ? address?.latitude : guestUserInfo?.latitude
    );
    formData.append("lng", token ? address?.lng : guestUserInfo?.longitude);
    formData.append(
      "longitude",
      token ? address?.longitude : guestUserInfo?.longitude
    );
    formData.append("guest_id", guestId);
    formData.append("house", address?.house);
    formData.append("floor", address?.floor);
    formData.append("city", token ? address?.city : guestUserInfo?.city);
    formData.append(
      "district",
      token ? address?.district : guestUserInfo?.district
    );
    formData.append("state", token ? address?.state : guestUserInfo?.state);
    formData.append(
      "country",
      token ? address?.country : guestUserInfo?.country
    );
    formData.append(
      "pincode",
      token ? address?.pincode : guestUserInfo?.pincode
    );
    formData.append(
      "contact_person_name",
      token ? address?.contact_person_name : guestUserInfo?.contact_person_name
    );
    formData.append("contact_person_number", contactPersonNumber);
  ...
  const handleOrderMutationObject = (carts, productList) => {
    ...
    if (getCurrentModuleType() === "pharmacy") {
      return returnFormData(carts, productList, guestId, isDigital, originData);
    } else {
      return moduleWiseFormData(
        carts,
        productList,
        guestId,
        isDigital,
  ...
  const paymentMethodErrorRes = (response) => {
    if (response.status === 203) {
      toast.error(response.data.errors[0].message);
    }
  };
  const handleOrderSuccessSecond = (productList) => {
    let totalQty = 0;
    ...
    const handleSuccessSecond = (response) => {
      if (response?.data) {
        if (paymentMethod === "digital_payment") {
          ...
          paymentMethodErrorRes(response);
          /* setOrderSuccess(true) */
        }
      }
    };
  ...
  const orderhandleSuccess = (productList) => {
    let totalQty = 0;
    let carts = handleProductList(productList, totalQty);
    ...
    if (isAvailable) {
      ...
      if (paymentMethod === "wallet") {
        if (Number(walletAmount) < Number(totalAmount)) {
          ...
          handleOrderSuccessSecond(productList);
        }
      } else {
        orderhandleSuccess(productList);
      }
    } else {
      toast.error(
        t(
 ...
 /*  const isStoreOpen = () => {
  ...
  const customerWalletBalanceSet = () =>{
    if (
      usePartialPayment &&
