...
const SearchResult = (props) => {
  ...
  const sortPageDataFunction = (onlyItems) =>{
    const sortPageData = [];
    if (sortBy === "high2Low") {
      sortPageData.push(getHighToLow(onlyItems));
    } else {
      sortPageData.push(getLowToHigh(onlyItems));
    }
    setPageData(sortPageData[0]);
  }
  const handleAPiCallOnSuccess = (res) => {
    ...
    if (currentTab === 0) {
      ...
      if (initialHigh2LowSortedData?.length > 0) {
        ...
        sortPageDataFunction(onlyItems)
      }
    } else {
      setTotalDataCount(res?.total_size);
      if (initialHigh2LowSortedData?.length > 0) {
  ...
  const createPageItems = (itemCategoryId,pageItems,item) =>{
    if (selectedCategories?.includes(itemCategoryId)) {
      pageItems?.push(item);
    }
  }
  const getPageItemsData = (item, pageItems) => {
    if (item?.category_ids?.length === 1) {
      const itemCategoryId = Number.parseInt(
        currentTab === 0 ? item?.category_ids?.[0]?.id : item?.category_ids?.[0]
      );
      createPageItems(itemCategoryId,pageItems,item);
    } else {
      item?.category_ids?.forEach((catItem, index) => {
        if (index != 0) {
          ...
          createPageItems(categoryId,pageItems,item);
    ...
    if (selectedCategories?.length >= 0) {
      if (pageData?.length > 0) {
        pageData?.forEach((item) => {
          getPageItemsData(item,pageItems)
        });
  ...
  const handleChangeRatings = (value) => {
    let newData = filterData.map((item, index) =>
      index === filterData.length - 1
        ? { ...item, rating: value, checked: value > 0 }
        : item
    );
    setFilterData(newData);
  };
