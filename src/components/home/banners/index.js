...
const Banners = (props) => {
        ...
        router.push(
          {
            pathname: "/product/[id]",
            query: {
              id: `${banner?.item?.slug || banner?.item?.id}`,
  ...
  const store_wise_banner = (banner) => {
    if (banner?.type === "store_wise") {
      router.push(
        {
          pathname: "/store/[id]",
          query: {
            id: `${banner?.store?.slug || banner?.store?.id}`,
            module_id: `${getModuleId()}`,
          },
        },
        undefined,
  ...
  const handleBannerClick = (banner) => {
    if (banner?.isCampaign) {
      ...
      store_wise_banner(banner);
    }
  };
  const handleModalClose = () => {
    setOpenModal(false);
  };
  const moduleLengthTypeGreater = () =>{
  ...
  const getModuleWiseBanners = () => {
    switch (getCurrentModuleType()) {
      case ModuleTypes.GROCERY:
        return moduleLengthTypeGreater();
      case ModuleTypes.PHARMACY:
       return moduleLengthTypeEqual();
