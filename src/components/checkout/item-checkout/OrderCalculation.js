...
const OrderCalculation = (props) => {
  ...
  const handleCouponDiscount = () => {
    ...
    if (couponDiscount && displayCouponDiscount) {
  ...
  const displayPartialPayment = usePartialPayment && payableAmount > walletBalance;
  const displayOrder = orderType === "delivery" || orderType === "schedule_order";
  const displayCouponDiscount = couponDiscount?.coupon_type === "free_delivery";
  return (
