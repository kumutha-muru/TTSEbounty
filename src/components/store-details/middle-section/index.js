...
const showHandleShimmerProducts = (
  ref,
  isLoading,
  state,
  isFetchingNextPage,
  ...
  const getCategoryWiseProduct = (products) => {
    const isAllExist = state.categoryId?.length === 0;
    if (isAllExist) {
    ...
    >
      ...
      <Grid item xs={12} container spacing={2}>
        {state.data &&
          ...
          getCategoryWiseProduct(state.data?.products)?.map((item,index) => {
            return (
              <Grid item key={index} xs={6} sm={4} md={3} lg={3}>
                <ProductCard
                  key={item?.id}
...
const MiddleSection = (props) => {
  const { storeDetails, ownCategories, isSmall, storeShare, configData } =
    props;
 ...
 /*  const handleSearchSuccess = (res) => {
  ...
  const checkOffsetCalculation = (initialHigh2LowSortedData,res) => {
    if (state?.data) {
      if (initialHigh2LowSortedData?.length > 0) {
        ...
        dispatch({
          ...
          payload: {
            ...res,
            products: getHighToLow(withoutDuplicacy),
          },
        });
      }
    } else {
  ...
  const handleSuccess = (res) => {
    if (res) {
      ...
      if (offset > 1) {
        checkOffsetCalculation(initialHigh2LowSortedData,res);
  ...
  const handleCategoryId = (id) => {
    ...
    dispatch({ type: ACTION.setCategoryId, payload: [...new Set(id)] });
  };
                ...
                <>
                  <Grid item xs={7} md={7.5}>
                    {getCurrentModuleType() === ModuleTypes.FOOD ? (
                      configData?.toggle_veg_non_veg === true ? (
                        <VegNonVegCheckBox
                          selected={state.type}
                          handleSelection={handleSelection}
                          checkState={checkState}
