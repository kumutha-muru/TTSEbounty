...
MainApi.interceptors.request.use(function (config) {
  ...
  let storeZoneId = undefined;
  const url = window.location.pathname.split("/store/");
  ...
  if (typeof window !== "undefined") {
    ...
    storeId = window.location.pathname === "/home" ? "" : url[1];
    storeZoneId =
      window.location.pathname === "/storeApply"
        ? localStorage.getItem("Store-Z-id")
        : "";
  }
  ...
  if (storeZoneId) config.headers.storeZoneId = storeZoneId;
