...
const maxWidthStyle = (horizontalcard, cardFor) => {
  const horizontalcardMaxWidth = horizontalcard === "true" ? "440px" : "320px";
  return cardFor === "list-view" ? "100%" : horizontalcardMaxWidth;
};
const marginStyle = (nomargin, cardType) => {
  const cardTypeMargin = cardType === "vertical-type" ? "0rem" : ".7rem";
  return nomargin === "true" ? "0rem" : cardTypeMargin;
};
const cardHeightStyle = (cardheight) => {
  return cardheight ? "130px" : "150px";
};
const horizondalCardWidth = (cardFor, cardWidth) => {
  return cardFor === "list-view" ? "100%" : cardWidth || "300px";
};
const wishlistcardNomargin = (nomargin) => {
  return nomargin === "true" ? "0rem" : ".4rem";
};
export const CardWrapper = styled(Card)(
  ({
    ...
    cursor: "pointer",
    maxWidth: maxWidthStyle(horizontalcard, cardFor),
    width:
      cardType === "vertical-type" || cardType === "list-view"
        ? "100%"
        : horizontalcard === "true" && "440px",
    margin: wishlistcard === "true" ? "0rem" : marginStyle(nomargin, cardType),
    borderRadius: "8px",
    ...
    [theme.breakpoints.down("sm")]: {
      height: horizontalcard !== "true" ? "320px" : cardHeightStyle(cardheight),
      width:
        horizontalcard === "true"
          ? horizondalCardWidth(cardFor, cardWidth)
          : "100%",
      margin: wishlistcard === "true" ? "0rem" : wishlistcardNomargin(nomargin),
    },
    [theme.breakpoints.up("sm")]: {
      height: cardheight || "330px",
...
const foodVariationsHandler=(e)=>{
  ...
  const withoutFoodModule = (itemObject) =>{
    if (item?.maximum_cart_quantity) {
      if (item?.maximum_cart_quantity <= isExisted?.quantity) {
        toast.error(t(out_of_limits));
      } else {
  ...
  const handleIncrement = () => {
    ...
    if (isExisted) {
      if (getCurrentModuleType() === "food") {
        foodModule(itemObject);
      } else {
        if (isExisted?.quantity + 1 <= item?.stock) {
          withoutFoodModule(itemObject);
        } else {
    ...
    return (
      ...
      >
        ...
        >
          ...
          {item?.wholesale_item_price?.length > 0 ? (
            <Typography variant={isSmall ? "body3" : "subtitle2"}>
              {t("start from")}
            </Typography>
          ) : (
