...
const Address = (props) => {
  ...
  const allSaveAddress = () => {
    return AllSaveAddress?.map((item) => {
      return (
        <Grid
          item
          key={item.id}
  ...
  const loadAllAddress = () =>{
    return AllSaveAddress && AllSaveAddress?.length > 0 ? (
      <Box>
        <Grid container>{allSaveAddress()}</Grid>
      </Box>
    ) : (
      <Stack
        alignItems="center"
    ...
    >
      ...
      <Grid container spacing={{ xs: 1, sm: 1.5, md: 2.5 }}>
        <Grid item xs={12} md={12}>
          <NoSsr>{isLoading ? <Shimmer /> : loadAllAddress()}</NoSsr>
        </Grid>
      </Grid>
    </CustomStackFullWidth>
  );
