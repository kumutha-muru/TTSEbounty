...
import { Skeleton, Typography, useTheme, Grid } from "@mui/material";
...
const ProductInformation = ({
  ...
  const incrementQuantity = () => {
    /* if (state.modalData[0]?.wholesale_item_price?.length > 0) { */
  ...
  const topInformation = () => {
    return (
      ...
      >
        ...
        {state.modalData[0]?.wholesale_item_price?.map((value) => {
          return (
            <Grid item xs={12} container key={value.id}>
              <Grid item xs={4}>
                <Typography
                  sx={{
