...
const CartContent = (props) => {
  ...
  const l = cartItem?.wholesale_item_price?.length - 1;
  ...
  const maximumCartQuantity = (itemObject) => {
  ...
  const foodBasedIncrement = (itemObject) => {
    if (getCurrentModuleType() !== "food") {
      if (cartItem?.stock <= cartItem?.quantity) {
        toast.error(t(out_of_stock));
      } else {
        maximumCartQuantity(itemObject);
      }
    } else {
      if (cartItem?.maximum_cart_quantity) {
        if (cartItem?.maximum_cart_quantity <= cartItem?.quantity) {
  ...
  const handleIncrement = (cartItem) => {
    ...
    const pricefilter = cartItem?.wholesale_item_price?.filter(function (value) {
      if (updateQuantity >= value.min_qty && updateQuantity <= value.max_qty) {
        return value;
      } else {
        return 0;
      }
    ...
    const price =
      cartItem?.wholesale_item_price?.length > 0
        ? pricefilter * updateQuantity
    ...
    const seletedOptionPrice =
      (cartItem?.selectedOption?.length > 0
        ? cartItem?.selectedOption?.[0]?.price
        : cartItem?.price) * updateQuantity;

    const withoutFoodProductPice =
      cartItem?.wholesale_item_price?.length > 0
        ? pricefilter * updateQuantity
        : seletedOptionPrice;
    const mainPrice =
      getCurrentModuleType() === "food" ? productPrice : withoutFoodProductPice;
    ...
    foodBasedIncrement(itemObject);
  };

  const handleDecrement = () => {
    const updateQuantity = cartItem?.quantity - 1;
    const pricefilter = cartItem?.wholesale_item_price?.filter(function (
      value
    ) {
      if (updateQuantity >= value.min_qty && updateQuantity <= value.max_qty) {
        return value;
    ...
    const price = cartItem?.wholesale_item_price[0]
      ? pricefilter * updateQuantity
      : cartItem?.price + getTotalVariationsPrice(cartItem?.food_variations);
 ...
 /*  const handleFoodItemTotalPriceWithAddons = () => {
  ...
  const quantityfilter = cartItem?.wholesale_item_price?.filter(function (
    value
  ) {
  ...
  return (
    <>
      ...
      >
        ...
        <CartIncrementStack>
          {cartItem?.quantity === 1 || cartItem?.quantity === quantityfilter ? (
            <IconButton
          ...
          <IconButton
            ...
            disabled={
              isLoading ||
              cartItem?.wholesale_item_price[l]?.max_qty === cartItem?.quantity
            }
          >
            <AddIcon
              sx={{
