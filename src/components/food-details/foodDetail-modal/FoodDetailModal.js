...
const FoodDetailModal = ({
  ...
  const optionalItemIndexFunction = (isTrue,selectedIndexCount,optionalItemIndex) =>{
      ...
      if (indexWithoutDuplicates.length > 0) {
        indexWithoutDuplicates.forEach((itemIndex) => {
          ...
          if (optionalItemIndex) {
            optionalItemIndexFunction(isTrue,selectedIndexCount,optionalItemIndex)
          } else {
            isTrue = true;
          }
        });
  ...
  const singleItemTypeFunction =(item,requiredItemsList,isExistInSelection) =>{
    let itemCount = 0;
    if (item.type === "single") {
      ...
      if (selectedOptions.length === 0) {
        ...
        requiredItemsList?.forEach((item, index) => {
          ...
          if (isExistInSelection) {
            singleItemTypeFunction(item,requiredItemsList,isExistInSelection);
          } else {
            handleRequiredItemsToaster(requiredItemsList, selectedOptions);
          }
        });
  ...
  const selectedOptionExistFunction = (
    option,
    optionIndex,
    choiceIndex,
    isRequired
    ...
    if (isItemExistFromSameVariation) {
      ...
      setTotalPrice(
        (prevState) =>
          prevState -
          Number.parseInt(isItemExistFromSameVariation.optionPrice) * quantity +
  ...
  const selectedVariationsFunction = (
    option,
    optionIndex,
    choiceIndex,
    isRequired
  ...
  const unSelectVariationsFunction = (option, choiceIndex) => {
    ...
    setTotalPrice(
      (prevState) => prevState - Number.parseInt(option.optionPrice) * quantity
    );
    setVarPrice(
      (prevPrice) => prevPrice - Number.parseInt(option.optionPrice) * quantity
    );
  };
  const multipleVariationsType = (
    e,
    option,
    optionIndex,
    choiceIndex,
  ...
  const changeChoices = (
    ...
    if (choiceType === "single") {
      if (checked) {
        ...
        unSelectVariationsFunction(option, choiceIndex);
      }
    } else {
      /* for multiple optional variation selection */
      multipleVariationsType(e, option, optionIndex, choiceIndex, isRequired);
    }
  };
